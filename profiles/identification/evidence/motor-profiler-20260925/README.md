...
## 四次运行统计（0.8 A 档，均为进行中的快照）

`tools/motor_profiler_identify.py` 可重复地把识别跑到"当前能达到的最远阶段"并留证。
四次的最终读数：

| 参数 | 均值 | 标准差 | 最小 | 最大 | 极差/均值 | 均值 vs 数据库 |
|---|---:|---:|---:|---:|---:|---:|
| `Rs` | 4.87512 Ω | 0.2172 | 4.69337 | 5.15688 | **9.5%** | **−7.9%** |
| `Ls` | 0.00089274 H | 6.21e-5 | 0.00082249 | 0.000954 | **14.7%** | −15.6% |
| `Ke` | 3.75248 | 0.7122 | 2.71462 | 4.33717 | **43.2%** | −24.4% |

**`Rs` 的极差/均值 9.5%，落在 EXP-09 通过标准的 10% 以内**；`Ls` 14.7% 超出；
`Ke` 43.2% 严重超出——因为 `Ke` 依赖识别是否走到磁链估计阶段，四次到达的阶段不同。

## 为什么 `SC_COMPLETED` 到不了 1

5 分钟长测（`identify_run4_0p8A.json`）实测的状态轨迹：

```text
t= 12.0s  state=2 (SCC_ALIGN_PHASE)
t= 38.9s  state=1 (SCC_DUTY_DETECTING_PHASE)
t= 65.7s .. 308.1s  state=1 不变, SC_COMPLETED=0
```

识别走到 **`SCC_DUTY_DETECTING_PHASE`（占空比检测）后稳定停住**，不再前进。
该阶段的职责是检出达到目标电流所需的占空比；它一直没有满足退出条件。

**这是 Profiler 预编译内核在该电机 + 该功率板组合上的行为**，不在 FluxRT 可控范围，
也不在我能修改的源码范围内（`SCC_*` 在 `libmp-G4-*.a` 里）。可能的原因（母线/限流、
绕组实际阻值相对该内核的期望区间、功率板分流/增益标定）需要另立课题排查。

## 当前可用的最佳估计（仍非最终值）

| 参数 | 建议取值 | 依据 | 可信度 |
|---|---:|---|---|
| `Rs` | **4.875 Ω** | 四次均值，极差 9.5% | 较高（在 EXP-09 10% 门内） |
| `Ls` | **0.000893 H** | 四次均值，极差 14.7% | 中（超出 10% 门） |
| `Ke` | 不宜采用 | 极差 43.2%，依赖到达阶段 | 低 |

**`Rs` 已经可以作为一个有实测支撑的值使用**；`Ls`/`Ke` 还需要更完整的识别才能定稿。
