...
> **一句话结论**：板上 Profiler 固件是**纯 4 字节、无负载**的 ASPEP 实现
> （见 §5）。它只接受不带负载的控制帧，因此**任何需要负载的 MCP 操作（含读寄存器）
> 都不可用**。修复点在固件的 `ASPEP_HWDataReceivedIT`，不在控制端。
...
### 5.0 最终判据：4 字节头 vs 8 字节带负载

**保持目标自由运行**（OpenOCD 存活，见下文的坑），每项测 5 次：

| 帧类型 | 接受率 | 结论 |
|---|---|---|
| **BEACON 头（4 字节，无负载）** | **5/5** | 这个固件接受无负载的 BEACON |
| BEACON 带负载（8 字节） | 2/5 | 负载会打乱流 |
| PING 头（4 字节） | 3/5 | 同样接受无负载形式 |
| PING 带负载（8 字节） | 2/5 | 同上 |

也就是说：**这个固件按"每帧恰好 4 字节"工作**。它自己发出来的帧也全是 4 字节
（`05 00 00 50`、`06 00 00 60`、`0f 04 04 c0`），完全自洽。

### 5.0.1 为什么这使全自动参数读取不可能

MCP 的寄存器读写必须携带负载：

- 读寄存器：`GET_DATA_ELEMENT(0x10)` + 2 字节 element ID = **4 字节负载**
- 写寄存器：命令 + element ID + 数据 = **更长的负载**

而设备收到 `DATA_PACKET` 时 `rxBuffer` 始终为全 0（实测），负载根本没进来，
于是按 `BAD_PACKET_SIZE` 或 `BAD_CRC_HEADER` 回 NACK。**因此读不到 `SC_RS` /
`SC_LS` / `SC_KE` 等任何识别结果。**

同样的原因让官方上位机也连不上：Motor Pilot 发的是带负载的控制帧。

### 5.1 直接观测

```text
Capabilities  = 0/7/7/32/0   (与 mcp_config.c 静态初始化一致, 正确)
ASPEPIp       = 0x20000a54
rxBuffer      = 0x20000fb4
maxRXPayload  = 0
ASPEP_State   = 0 (IDLE)
rxBuffer      = 00 00 00 00 00 00 00 00    <<< 负载从未写入
```
