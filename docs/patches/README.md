...
`motorID=1` 时四种寄存器（`SC_VBUS`/`SC_RS`/`SC_LS`/`SC_KE`）返回的都是
`5a 00 00 c0 00 00 00 00 00`，即负载 `00 00 00 00 00`。

对这个响应有两种解读，**目前无法判定哪一种**：

1. **这是 `MCP_CMD_NOK` 的错误上报**：头部 `5a 00 00 c0` 解出长度字段 5、
   类型 `0xA`；负载末尾可能是错误码。
2. **这是合法的"值为 0.0"响应**：负载前 4 字节是 `float(0.0)`，第 5 字节是
   `MCP_CMD_OK`。若如此，则 `0.0` 是**正确**的——因为这些寄存器只有在跑过一次
   参数识别之后才会被写入，当前设备从未完成过识别，所以全是 0。

**线索**：`GET_MCP_VERSION` 的响应是 `5a 00 00 c0 01 00 00 00 00`，与上面同型，
而它的值恰好是 `1`（`MCP_VERSION` 确实是 1）。这**支持解读 2**。

**下一步**：跑一次参数识别，再看这些寄存器是否变成非零。若变成非零，则解读 2
成立、读取路径完全正确；若仍是 0，则要按解读 1 继续查请求编码。
