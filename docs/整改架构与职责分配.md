...
[ST/VESC 工程修正路线](ST与VESC工程改进路线图.md)回答“后续要改什么”，
本文回答以下问题：
...
Rust target_math ──窄数学端口──→ MCU CORDIC/FPU/DSP 适配器
...
│  ├─ foc_production_profile.c/.h    已实现，编译期只读身份/CRC/审批门
│  └─ foc_calibration_service.c/.h   规划新增，辨识命令和人工批准流程
│
├─ profiles/
│  ├─ schema/                        已实现，候选记录格式契约
│  ├─ candidates/                    已实现，完整参数、身份、审批和预期 CRC
│  ├─ generated/                     已实现，仅供评审、不自动编入固件的 C 片段
│  ├─ identification/                已实现 session/evidence/report；不足时拒绝候选
│  └─ calibration/                   已实现目录规则；A19 才写入真实板卡标定记录
│
├─ tools/
│  ├─ foc_profile_tool.py            已实现，离线校验、证据门和候选片段生成
│  ├─ foc_identification_tool.py     已实现，采集证据和参数筛查；不写固件
│  └─ foc_calibration_tool.py        已实现，相电压记录/哈希/审批拒绝门
│
├─ foc/
│  ├─ include/
│  │  ├─ foc_build_profile.h         已实现，三档主宏与能力派生的唯一契约
│  │  ├─ foc_types.h                 现有，最小跨语言定宽数据类型
│  │  ├─ foc_rust_bridge.h           现有，唯一 Rust C ABI
│  │  ├─ foc_platform.h              现有，MCU 平台公共契约
│  │  ├─ foc_phase_voltage_capture.h 已实现，Diagnostic 固定窗纯 C 契约
│  │  ├─ foc_phase_voltage_model.h   已实现，名义/逐板标定来源与诊断换算契约
...
│  │  ├─ foc_phase_voltage_capture.c 已实现，256 拍无分配原始码缓冲
│  │  └─ foc_phase_voltage_model.c   已实现，整数 raw→mV 与模型一致性校验
│  └─ platform/stm32g431/
...
│     └─ foc_math_accel_stm32g431.c   现有，CORDIC + FPU 数学后端
│
├─ rust/crates/
│  ├─ foc-algorithm/src/
...
│  │  ├─ inverter.rs                  已实现，死区/器件压降/软零带纯模型
...
│  │  ├─ voltage.rs                   已实现 CommandModel 与默认关闭的逆变器模型组合；目标接线/融合待做
...
│     ├─ scheduler.rs                 已实现 PC 整数多速率、占空保持和生效延迟
...
A14 已先落地 `foc_production_profile.c/.h`：所有构建档启动时都校验
schema/revision、板卡 ID、电机 ID、完整运行配置 CRC、档案 CRC 和审批位依赖；
校验失败时不提交配置且不绑定平台。它是未来 `foc_parameter_service`
的“编译期只读来源”，尚未实现 Flash 双副本、版本迁移或掉电恢复。

A15 已把 A14 revision 1 的 60 个运行配置 ABI 字保存为 `profiles/candidates` 下的完整
候选，并提供 JSON Schema、Rust 源值导出器、离线 CRC/证据校验器和未编译的生成片段。
生成工具属于开发侧，不进入 ISR/固件，也不能设置审批；只有人工复核后才允许把片段
集成到 `foc_production_profile.c`。具体规则见
[`参数候选与审批流程.md`](parameters/参数候选与审批流程.md)。

A16 在同一开发侧边界增加 `profiles/identification` 与离线筛查工具。它只读取已落盘
CSV/日志并验证固件、工况、安全包络和 SHA-256；当前 50 Hz trace 缺实测端电压、高频
电流瞬态、绕组温度和独立转速真值，因此只输出 `Rs` 趋势并拒绝 revision 2。详细边界
见 [`参数辨识采集与筛查流程.md`](parameters/参数辨识采集与筛查流程.md)。

A17 增加 `foc_phase_capture start|stop|status|dump` 管理入口，但应用层不配置 ADC、不读取
寄存器，也不把原始码转成控制反馈。PC 采集脚本只负责停机包络、导出和证据校验；原始
码标定、有效性和后续参数估计仍属于平台层与 Rust 辨识层的后续工作。

构建能力也按职责隔离：`build.ps1` 与 `custom.cmake` 选择
Diagnostic/Calibration/Production 和 Rust 优化等级；`foc_build_profile.h` 统一派生在线
调参、trace、相电压固定窗和数学诊断能力。Calibration 只允许停机固定窗，并在应用层和
平台层同时拒绝 arm；Production 不包含固定窗。任何档位都不能借构建宏删除关断、输出
复核、deadline 或硬件保护。构建档故意不写进共享 `.config`，硬件/算法候选仍由
Env/SCons/Kconfig 生成，档位由独立 CMake cache 和 Rust target 目录隔离。当前实现见
[`构建档与优化等级.md`](构建档与优化等级.md)。这项“路线阶段 1.1”工作不等于下表的架构
工作包 A1 已完成。
...
A17 已先落地 `foc_phase_voltage_capture.c`：它同样不依赖 STM32，但语义是“一次短窗”，
不是连续 trace。ISR 在 armed 状态顺序写 256 个样本，满后自动停止；线程只有停止后才能
读取。4096 B 样本区只在 Diagnostic 平台实例化，Production 不承担这部分 RAM。
...
A17 当前仍在聚合平台文件中完成 PC0/PC3/PC1 注入序列与 PC9 `IO_BEMF` 控制；它只
输出 ADC 原始码。电机未 arm 才能启动采集，采集 armed 时控制启动会被拒绝。后续结构
拆分时寄存器配置迁入 `foc_stm32g431_phase_voltage.c`，通用固定窗不随 MCU 迁移。

A19.1 增加了 3.3 V、12 bit、10 kΩ/2.2 kΩ 的 ST 官方**名义**换算模型。该模型由平台
层发布，带 `NOMINAL_COMPONENTS | DIAGNOSTIC_ONLY`，不带 `BOARD_CALIBRATED` 或
`OBSERVER_ELIGIBLE`。因此它可以用于 Shell/CSV 对拍，但不能越过后续质量门送入 Rust
快照；默认观察器仍走 `foc-control/voltage.rs` 的 `CommandModel`。

CM2 已按本文分配完成纯公式与状态层迁移：`foc-algorithm/inverter.rs` 是死区、
等效器件压降和软零带公式的唯一 Rust 实现；`foc-control/voltage.rs` 持有相电流
一阶滤波状态，组合观测器等效 PWM 与输出前馈。CM3 进一步在 `foc-rt-bridge`
增加 V13/V7 稳定 C ABI，target 与 Host 现在持有同样的模型状态；总门、观测修正、
PWM 前馈是独立门，默认 `0/0/0`。无功率 S4 已按该边界完成 stage `0/1/2/3/4/0`
配置与标签核对；管理命令暴露的栈溢出通过 tshell 单写静态暂存区修正，没有扩大
FinSH 栈，也没有把可变管理状态放入 ISR。详见
...
| bridge 中 Host-only 死区补偿 | `algorithm/inverter.rs` + `control/voltage.rs` | CM2 已迁移公式/状态；CM3 已以 V13/V7 分层接入 target，默认 `0/0/0`，无功率 S4 已完成，带功率 WCET/A-B 待做 |
...
V5 已按该规则开始迁移：新增的运行期观测器保持门没有继续作为裸字段散落在尾部，而是
追加为 C/Rust 对称的 `foc_observer_run_reliability_config_t` /
`FocObserverRunReliabilityConfig`。为控制本次风险，既有 V4 扁平字段没有整体重排；后续
新增字段继续进入对应领域结构，待有独立迁移窗口时再统一收拢旧字段。
...
A14 当前只实现了上图前两层之间的最小安全骨架：编译默认值通过一份
40 B 只读档案绑定完整运行配置。当前 `approval_flags=0`，因此即使档案
身份和 CRC 全部通过，也不会授权闭环。详见
...
当前落位：`foc-sim/src/scheduler.rs` 与 Simulink 已拆分 `pwmTs/controlTs` 并建模
占空保持/延迟；目标端已有 `foc_pwm_timing` 纯契约和 STM32G431 平台实现。默认路径为
12/12 kHz 直接 OC4REF；24/12 kHz Diagnostic 候选使用 TIM1 OC4REF 经 TIM2 ITR0
二分频后触发 ADC。旧 `TIM1 Update -> ADC` 组合已被 Host 契约拒绝，因为 S5 证明它
落在无效三分流采样窗口。修正版开环量级已与默认对齐，但闭环只完成 1/3 次，因此仍
默认关闭。目标端没有复用 PC 调度器的 `Vec`/报告逻辑，也没有在 ISR 中用浮点时间累加。
...
| A2 多速率（受限 S5 完成，未升格） | 24 kHz PWM + 12 kHz 控制候选 | `foc-sim/scheduler.rs`、`foc_pwm_timing`、STM32 TIM1 OC4REF→TIM2 ITR0 分频→ADC | PC/Host/目标构建、S4、受限开环通过；闭环 1/3 且无独立真值，候选保持关闭 | A1 |
| A3 快环优化 | Clarke 复用、数学后端、trace 裁剪 | `control/signal.rs`、`bridge/target_math.rs`、Kconfig | 数值误差、平均/最坏 cycles | A1 |
| A4 逆变器模型（CM4.2 受限完成，A27 扩展中） | 死区、器件压降和观测器电压源 | `algorithm/inverter.rs`、`control/voltage.rs`、`bridge/lib.rs`、`sim/lib.rs` | stage 2 下已修正捕获/接管并解耦转矩支撑与 PI 预装；582 rpm 完成 3/3 冷启动和 30 s S5，峰值观察速度由 894 降到 722 rpm。仍无独立真值，stage 2 不升格为 Production；下一门是多转速/方向重复 | A1、A3 |
...
| A6 相电压（A19.1 名义诊断模型已落地，未升格） | PC0/PC3/PC1 原始码和 ST 名义 10k/2.2k 换算已完成；逐板标定、有效性和混合电压源待做 | `foc_phase_voltage_capture/model`、STM32G431 平台；后续快速输入 ABI、`control/voltage.rs` | Host/三档构建/S4 元数据通过；名义模型明确 observer-disabled，仍缺动态相序、运行 WCET和失效回退 | A2、A4 |
| A7 控制质量（角补偿接口已落地、未升格） | dq 解耦、限幅、角度/温度补偿 | 角补偿现位于 `foc-control/controller.rs` + bridge V6 config；后续随结构拆分迁入 `control/voltage.rs` | 同源仿真已扫正负系数；非零候选跨工况不一致，仍需 GPIO 时序、正反转和编码器真值 | A4、独立真值 |
...
- [后续任务阶段计划](后续任务阶段计划.md)：统一维护 A0～A28 的完成历史、状态、依赖、仪器和完成门；
- [项目操作记录](工程操作日志.md)：记录每次修改、验证、Git 关联、风险和下一步；
- 本文：决定模块、目录、所有权、依赖和工作包落点；
- [整改路线](ST与VESC工程改进路线图.md)：决定优先级、功能内容和验收门；
- [现有 C/Rust 混合架构](C与Rust混合架构.md)：说明当前已经接通的调用链；
- [架构与安全边界](架构与安全边界.md)：记录当前必须保持的实机安全不变量；
- [硬件数学加速](硬件数学加速与CPU回退.md)：约束 CORDIC 和 CPU 回退；
- [仿真与实机相关性](仿真实机相关性验证.md)：定义模型与实机证据如何对齐。
